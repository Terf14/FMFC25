...
if (!isset($_SESSION['user_id'])) {
    // If not logged in, redirect to login page with an error
...
try {
    ...
    // 1. Fetch all players from the 'players' table for the current user
    $stmt_players = $conn->prepare("SELECT * FROM Players WHERE user_id = ?");
    $stmt_players->execute([$user_id]);
    $current_players = $stmt_players->fetchAll(PDO::FETCH_ASSOC);
        ...
        if (!$total_player_data) {
            ...
            $stmt_update = $conn->prepare("
                UPDATE player_total_stats SET
                    ...
                    status = ?,
                    injured = ?,
                    injured_count = ?, -- แก้ไข: อัปเดตค่าล่าสุดโดยไม่บวกเพิ่ม
                    is_academy_product = ?,
    ...
    // 3. Reset stats in the 'players' table for the current user
    ...
    $_SESSION['toast_message'] = [
        'type' => 'success',
        'message' => 'รวมสถิตินักเตะและรีเซ็ตข้อมูลปัจจุบันสำเร็จ!'
    ];
    header("Location: total_stats.php"); // Redirect to the new total stats page
    exit();
    ...
    header("Location: total_stats.php"); // Redirect back to total stats page on error
    exit();
}
?>